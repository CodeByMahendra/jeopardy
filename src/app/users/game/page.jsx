...
//     try {
//       const res = await fetch("/api/auth/token", { credentials: "include" }); // ✅ Send request with cookies
...
export default function QuizGame() {
  ...
  // ✅ Function to fetch token from API route
  const fetchToken = async () => {
    try {
  ...
  useEffect(() => {
    const initializeGame = async () => {
      const storedUser = localStorage.getItem("user");
      const authToken = await fetchToken(); // ✅ Fetch token from cookies
